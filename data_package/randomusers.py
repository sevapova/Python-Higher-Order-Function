...
def get_full_names(data: dict) -> list[str]:
    ...
    """
    return [user["name"]["first"] + " " + user["name"]["last"] for user in data["results"]]

x = get_full_names(randomuser_data)
print(x)
...
result = get_users_by_country(randomuser_data, "India")
print(result)
...
def sort_users_by_age(data: dict, descending: bool = False) -> list[dict]:
    ...
    return [
        {"name": user["name"]["first"] + " " + user["name"]["last"], "age": user["dob"]["age"]}
        for user in sorted_users
    ]
young_to_old = sort_users_by_age(randomuser_data)
print(young_to_old)

old_to_young = sort_users_by_age(randomuser_data, descending=True)
print(old_to_young)
...
nationality_counts = group_users_by_nationality(randomuser_data)
...
coords = get_all_coordinates(randomuser_data)
print(coords)
...
def get_oldest_user(data: dict) -> dict:
    ...
    """
    oldest = max(data["results"], key=lambda user: user["dob"]["age"])
...
oldest_user = get_oldest_user(randomuser_data)
print(oldest_user)
...
result = find_users_in_timezone(randomuser_data, "+5:30")
print(result)
...
users_before_2010 = get_registered_before_year(randomuser_data, 2010)
print(users_before_2010)
